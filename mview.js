...
var MView = function(settings) {
  ...
  jQuery('head').append('<link rel="stylesheet" type="text/css" href="' + settings.resurl + 'style.css"/>');
...
MView.postLoad = function(el) {
  jQuery(el).find('.mv-u.muted').append('<img class="mv-icon mv-icon-muted" src="' + settings.resurl + 'img/muted_self.png" alt=[m]"/>');
  jQuery(el).find('.mv-u.deafened').append('<img class="mv-icon mv-icon-deafened" src="' + settings.resurl + 'img/deafened_self.png" alt=[d]"/>');
};
/*
MView.postLoad = function(el) {
  jQuery(el).find('.mv-u.deafened').append('<img class="mv-icon mv-icon-deafened" src="' + settings.resurl + 'img/deafened_self.png" alt=[d]"/>');
...
MView.getChanHTMLCodeFor = function(json) {
  ...
  if (json.name!='Root'){
html.append(jQuery('<div class="mv-c-name"/>').append(jQuery('<p/>').append(json.name)));}
...
MView.getUserHTMLCodeFor = function(json) {
  var el = jQuery('<li/>');
  el.addClass('mv-u');
  if (json.selfMute || json.mute ) {
    el.addClass('muted');
  }
  if (json.selfDeaf || json.deaf) {
    el.addClass('deafened');
  }
  el.append('-' + json.name);
  return el;
